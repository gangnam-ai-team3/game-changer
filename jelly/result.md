...
게임명을 알 수 있으면 결과 맨 위에 타이틀로 표시합니다. 여러 게임이 섞여 있으면 함께 표시하고, 게임명을 알 수 없으면 생략합니다.
...
이슈 안에 '위험'으로 분류된 근거가 있으면, 그 위험이 특정 언어에서만 나오는지 근거 언어 전체에 공통인지 나눠서 경고로 표시합니다(규칙은 `stub.md`의 "위험 신호의 언어 범위" 참고). 위험으로 분류된 근거 옆에는 "문화권 특이" 또는 "공통" 표시를 함께 붙입니다.
...
- <콘텐츠 1>
  - <이슈 1>
    ...
    - 근거: <근거 1>, <근거 2>
    - 개선: <개선 방향 1>, <개선 방향 2>
    - (위험 근거가 일부 언어에서만 있는 경우) ⚠ <언어>에서만 위험 신호가 확인되어 별도 대응이 필요합니다.
    - (위험 근거가 근거 언어 전체에 공통인 경우) ⚠ 근거가 있는 모든 언어권(<언어 목록>)에서 공통으로 위험 신호가 확인되어, 특정 언어가 아닌 전체 차원의 대응이 필요합니다.

## 4. 종합
전체 동향 요약을 먼저 짧게 설명한 뒤, 원인들을 개선 테마로 묶어 근거가 많고 시급한 것부터 순서대로 2~4개 항목(제목 + 1~2문장 설명)으로 나눠 우선순위 순서도 형태로 제시합니다.
개별 개선 문장을 그대로 나열하지 않습니다. 위험 신호는 특정 언어에서만 있는지 근거 언어 전체에 공통인지 나눠서 마지막에 설명합니다.

- <전체 동향 요약 한 문장>
- 개선 방향 (우선순위 순서)
  1. <제목 1> — <설명 1>
  2. <제목 2> — <설명 2>
- (위험 신호가 일부 언어에서만 있는 경우) <특정 언어권 설명>
- (위험 신호가 근거 언어 전체에 공통인 경우) <전체 차원 대응 필요 설명>
