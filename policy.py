...
RISK_SPECS = {
    ...
    RiskCategory.OPAQUE_PROGRESS: (
        ...
        "지출과 플레이를 반복해도 남은 비용과 횟수를 알 수 없어 통제감을 잃을 수 있습니다.",
    ...
    RiskCategory.RANDOM_BONUS: (
        ...
        Severity.HIGH,
        [PersonaKind.VALUE_SEEKING, PersonaKind.COLLECTOR],
        "확률형 보너스로 같은 금액을 지출한 이용자의 진행량이 달라져 공정성 우려가 커질 수 있습니다.",
    ...
    RiskCategory.EXPIRING_CURRENCY: (
        ...
        "남은 재화가 환불이나 전환 없이 삭제되어 일정이 불규칙한 이용자가 손실을 볼 수 있습니다.",
...
REVISION_SPECS = {
    RiskCategory.DOUBLE_GACHA: (
        "2단계 확률 제거",
        "Loot Cache와 Prime Parcel을 하나의 Cargo 직접 보상 구조로 통합합니다.",
        "모든 목표 보상의 단일 단계 확률과 최대 획득 비용을 화면에서 확인 가능",
    ),
    RiskCategory.FRAGMENTED_FLOW: (
        "이벤트 허브 통합",
        "구매, 개봉, 획득 보상, 토큰, 제작 진행을 한 Supply Bay 화면에 모읍니다.",
        "목표 보상 제작까지 필요한 핵심 동작을 한 화면에서 완료",
    ),
    RiskCategory.OPAQUE_PROGRESS: (
        "확정 마일스톤 추가",
        "개봉 횟수별 토큰, 재료, 최종 보상 마일스톤을 사전에 공개합니다.",
        "모든 유료 경로에 보이는 진행도와 고정 상한 존재",
    ),
    RiskCategory.RANDOM_BONUS: (
        "보너스 결정성 강화",
        "확률형 보너스 일부를 개봉당 고정 토큰과 누적 마일스톤으로 교체합니다.",
        "동일 지출의 최소 진행량 편차 0",
    ),
    RiskCategory.EXPIRING_CURRENCY: (
        "잔여 재화 보호",
        "종료 후 유예 기간을 두고 남은 토큰을 상시 재화로 자동 전환합니다.",
...
def decide(
    ...
    *,
    ...
    if analysis_incomplete:
        return Decision.HOLD, "새 자료의 AI 해석이 완료되지 않아 판단을 보류합니다."
    if any(risk.severity == Severity.CRITICAL for risk in risks):
        return Decision.HOLD, "검증된 Critical 위험이 있어 출시 판단을 보류합니다."
    insufficient = sum(not sample.sufficient for sample in samples)
    if insufficient >= 3:
        return Decision.HOLD, "세 언어권 이상이 최소 표본에 미달해 판단 근거가 부족합니다."
    if any(risk.severity == Severity.HIGH for risk in risks):
        return Decision.REVISE, "검증된 High 위험을 수정한 뒤 재검토해야 합니다."
    if insufficient:
        return Decision.REVISE, "일부 언어권 표본을 보강한 뒤 출시 판단을 갱신해야 합니다."
    return Decision.GO, "필수 표본을 충족했고 High 이상으로 검증된 위험이 없습니다."
