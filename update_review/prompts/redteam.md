...
- 각 `failure_path`는 해당 `risk_id`의 `prospective_templates.failure_path_by_risk_id` 중 하나를 원형 그대로 선택하라. 새 문장을 만들거나 변형하지 마라.
- 한국어 설명을 작성하되 제품명·ID는 원형을 유지하라.
- 출시 후 실제 이용자 반응·시점·완료 결과를 쓰지 마라.
- 요구된 structured_output tool로만 응답하라.

`risks`만 반환하고, 각 항목에서는 `risk_id`, `title`, `failure_path`, `revision_question`, `evidence_ids`, `validation_metric_ids`만 제공하라.
